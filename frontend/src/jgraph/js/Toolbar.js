 ...
 */
function Toolbar(editorUi, container, proposalViewMode) {
  this.editorUi = editorUi;
  this.container = container;
  this.staticElements = [];
  this.init(proposalViewMode);
 ...
 */
Toolbar.prototype.dropdownImage = IMAGE_PATH + "/expanded.gif";
 ...
 */
Toolbar.prototype.init = function (proposalViewMode) {
  var sw = screen.width;

  // Takes into account initial compact mode
  sw -= screen.height > 740 ? 56 : 0;

  /** @version Docstrail hide if in Review mode. */
  if (proposalViewMode !== Window.jgraphChannel.JMode.REVIEW) {
    if (sw >= 700) {
  ...
  this.editorUi.editor.addListener("computeZoomPercentLabel", this.updateZoom);

  /** @version Docstrail Do not add remaining toolbar items when in Review mode. */
  if (proposalViewMode === Window.jgraphChannel.JMode.REVIEW) return;
