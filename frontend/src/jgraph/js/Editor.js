...
Editor = function (chromeless, themes, model, graph, editable) {
  ...
  /**
   * @version Docstrail default file name should always be "Untitled proposal".
   */
  this.getOrCreateFilename = function () {
    return this.fileName || "Untitled proposal";
 ...
 */
Editor.checkmarkImage = IMAGE_PATH + "/checkmark.gif";
...
/**
 * Sets the XML node for the current diagram.
 * @version Docstrail, autosave is always on.
 */
...
/**
 * Sets the XML node for the current diagram.
 * @version Docstrail (Optional change - for safety only) Ensures gridEnabled is not set here.
 */
...
/**
 * Sets the XML node for the current diagram.
 * @version Docstrail Ensures grid enabled is not set here.
 */
 ...
 */
var PrintDialog = function (editorUi, title, data) {
  this.create(editorUi, title, data);
};

/**
 * Constructs a new print dialog.
 */
PrintDialog.prototype.create = function (
  editorUi,
  title,
  screenshotPositionData
) {
  ...
  function preview(print, screenshotPositionData) {
    if (screenshotPositionData) {
      pageCountCheckBox.checked = false;
      onePageCheckBox.checked = true;
    }
    ...
    if (autoOrigin) {
      ...
      if (!isNaN(pageCount)) {
        pageCount = screenshotPositionData ? 1 : pageCount;
    ...
    if (screenshotPositionData) {
      const div = preview.getSvg(screenshotPositionData);
      Window.jgraphChannel.emitSvg(div);
      return;
    }
    preview.open();
  ...
  if (screenshotPositionData) {
    preview(false, screenshotPositionData);
  }
