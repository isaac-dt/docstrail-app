...
EditorUi = function (editor, container, lightbox) {
  ...
  this.container = container || document.getElementById("jgraph-canvas");
  ...
  // Listen for requests to load graph.
  Window.jgraphChannel.subscribe((event, data) => {
    if (event === Window.jgraphChannel.JEvent.LOAD_DIAGRAM) {
      if (data.refreshCanvas) this.loadDiagram(data.xml, data.name);
    }
  });

  /** @version Docstrail implement autosave hook. */
  var autosaveMgr = new mxAutoSaveManager(this.editor.graph);
  autosaveMgr.save = () => {
    Window.jgraphChannel.emitAutosaveRequest();
  };
  /** @version Docstrail performs a save -- originates from autosave request. */
  Window.jgraphChannel.subscribe((event, data) => {
    if (event === Window.jgraphChannel.JEvent.DO_AUTOSAVE) {
      this.save(this.editor.getFilename() || this.editor.getOrCreateFilename());
    }
  });

  /** @version Docstrail Updates sidebar related styles for dynamic changes. */
  const refreshSidebarStyles = () => {
    this.hsplitPosition = this.getHsplitPosition();
    this.sidebarContainer.style.width = this.getSidebarContainerWidth();
    this.sidebarContainer.style.paddingLeft =
      Window.jgraphChannel.state.sidebarPaddingLeftPx + "px";
    this.sidebarContainer.style.left =
      Window.jgraphChannel.state.sidebarWidthPx + "px";
    this.refresh(true);
  };

  /** @version Docstrail renames the diagram and saves it. */
  Window.jgraphChannel.subscribe((event, data) => {
    if (event === Window.jgraphChannel.JEvent.RENAME_PROPOSAL) {
      this.editor.setFilename(data.name);
      this.saveFile();
    } else if (event === Window.jgraphChannel.JEvent.RENAME_PROPOSAL_MODAL) {
      var dlg = new FilenameDialog(
        this,
        this.editor.getFilename() || this.editor.getOrCreateFilename(),
  ...
  /** @version Docstrail Changes the sidebar width based on proposal view mode. */
  Window.jgraphChannel.subscribe((event, data) => {
    if (event === Window.jgraphChannel.JEvent.MODE_CHANGED) {
      if (data.mode === Window.jgraphChannel.JMode.EDIT) {
        Window.jgraphChannel.state.sidebarPaddingLeftPx =
          Window.jgraphChannel.constant.SIDEBAR_PADDING_SMALL;
        this.editor.graph.zoomTo(1);
      } else if (data.mode === Window.jgraphChannel.JMode.REVIEW) {
        Window.jgraphChannel.state.sidebarPaddingLeftPx =
          Window.jgraphChannel.constant.SIDEBAR_PADDING_LARGE;
        this.editor.graph.zoomTo(1);
      }
  ...
  /** @version Docstrail Destroys and rebuilds the toolbar based on proposal view mode. */
  Window.jgraphChannel.subscribe((event, data) => {
    if (event === Window.jgraphChannel.JEvent.MODE_CHANGED) {
      const newToolbar = this.createToolbar(
        this.createDiv("geToolbar"),
        data.mode
      );
      const oldToolbar = this.toolbarContainer.firstChild;
      if (oldToolbar) this.toolbarContainer.removeChild(oldToolbar);
      if (data.mode === Window.jgraphChannel.JMode.EDIT) {
        this.toolbarContainer.appendChild(newToolbar.container);
      } else if (data.mode === Window.jgraphChannel.JMode.REVIEW) {
        this.toolbarContainer.appendChild(newToolbar.container);
      }
      /** @version Docstrail Docstrail-introduced event: to recompute and update the zoom percentage label. */
      this.editor.fireEvent(new mxEventObject("computeZoomPercentLabel"));
    }
  });

  /** @version Docstrail Toggle menu bar based on proposal view mode. */
  Window.jgraphChannel.subscribe((event, data) => {
    if (event === Window.jgraphChannel.JEvent.MODE_CHANGED) {
      const child = this.menubarContainer.firstChild;
      if (data.mode === Window.jgraphChannel.JMode.EDIT) {
        if (child) this.menubarContainer.removeChild(child);
        this.menubarContainer.appendChild(this.menubar.container);
      } else if (data.mode === Window.jgraphChannel.JMode.REVIEW) {
        if (child) this.menubarContainer.removeChild(this.menubar.container);
      }
    }
  });

  /** @version Docstrail Update graph view configurations on proposal view mode change. */
  Window.jgraphChannel.subscribe((event, data) => {
    if (event === Window.jgraphChannel.JEvent.MODE_CHANGED) {
      if (data.mode === Window.jgraphChannel.JMode.EDIT) {
  ...
  /** @version Docstrail Subscribe to screenshot capture.*/
  Window.jgraphChannel.subscribe((event, data) => {
    if (event === Window.jgraphChannel.JEvent.CAPTURE_SCREENSHOT) {
      const dialog = new PrintDialog(ui, null, data);
      ui.showDialog(dialog.container, 300, 180, true, true);
      ui.hideDialog();
    }
  });
 ...
 * @version Docstrail
 */
EditorUi.prototype.titleHeight = 40;
...
EditorUi.prototype.hsplitPosition =
  screen.width <= 640
    ? Window.jgraphChannel.state.sidebarWidthPx +
      Window.jgraphChannel.state.sidebarPaddingLeftPx +
      118
...
/** @version Docstrail new-code: compute the hsplitPosition */
EditorUi.prototype.getHsplitPosition = () => {
      ...
      var updateChromelessToolbarPosition = mxUtils.bind(this, function () {
        ...
        if (graph.isViewer()) {
          this.chromelessToolbar.style.top =
            this.titleHeight + this.menubarHeight + "0";
        } else {
 ...
 * Loads a new diagram.
 */
EditorUi.prototype.loadDiagram = function (xml, filename) {
  try {
    var doc = mxUtils.parseXml(xml);
    this.editor.setGraphXml(doc.documentElement);
    this.editor.setModified(false);
 ...
 * @version Docstrail Always show the name docstrail as the title.
 */
...
EditorUi.prototype.refresh = function (sizeDidChange) {
  ...
  /** @version Docstrail adjust handle position */
  ...
  if (this.menubar != null) {
    this.menubarContainer.style.height =
      this.titleHeight + this.menubarHeight + "px";
    tmp += this.titleHeight + this.menubarHeight;
  }
  ...
  /** @version Docstrail Use docstrail-set styles to calculate the sidebar width. */
...
/** @version Docstrail computes the sidebar container width */
EditorUi.prototype.getSidebarContainerWidth = () => {
...
EditorUi.prototype.createDivs = function () {
  this.titleContainer = this.createDiv("geTitleContainer");
  ...
  /** @version Docstrail modified styles. */
  // Sets static style for containers
  this.titleContainer.style.position = "absolute";
  this.titleContainer.style.top = "0px";
  this.titleContainer.style.left =
    Window.jgraphChannel.state.sidebarWidthPx + "px";
  this.titleContainer.style.right = "0px";
  this.titleContainer.style.zIndex = "2";
  this.menubarContainer.style.top = this.titleHeight + "px";
  ...
  this.sidebarContainer.style.left =
    Window.jgraphChannel.state.sidebarWidthPx + "px";
  /** @version Docstrail add padding to the left of the sidebar container. */
  this.sidebarContainer.style.paddingLeft =
    Window.jgraphChannel.state.sidebarPaddingLeftPx + "px";
...
/**
 * Creates a new toolbar for the given container.
 * @version Docstrail @param {JMode} proposalViewMode
 */
EditorUi.prototype.createToolbar = function (container, proposalViewMode) {
  return new Toolbar(this, container, proposalViewMode);
};
...
/**
 * Adds the label menu items to the given menu and parent.
 * @version Docstrail Do not show popup for name editing. Save directly
 * with default name if none is found.
 */
EditorUi.prototype.saveFile = function (forceDialog) {
  const name = this.editor.getFilename();
  if (name) {
    this.save(name);
  } else {
...
EditorUi.prototype.save = function (name) {
  if (name != null) {
    ...
    if (xml.length < MAX_REQUEST_SIZE) {
      Window.jgraphChannel.emitSaveGraph(xml, name);
    } else {
