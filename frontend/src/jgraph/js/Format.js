 ...
 * Adds the label menu items to the given menu and parent.
 */
StyleFormatPanel.prototype.addSvgStyles = function (div) {
  var ui = this.editorUi;
  var graph = ui.editor.graph;
    ...
    var addButton = mxUtils.bind(this, function (colorset) {
        ...
        mxUtils.bind(this, function (evt) {
          ...
          try {
            ...
            for (var i = 0; i < cells.length; i++) {
              ...
              var defaults = graph.getModel().isVertex(cells[i])
      ...
      if (colorset != null) {
        ...
        if (colorset["gradient"] != null) {
          ...
          btn.style.backgroundColor = mxUtils.getValue(
            graph.defaultVertexStyle,
            mxConstants.STYLE_FILLCOLOR,
            "#ffffff"
          );
  ...
  if (this.format.currentScheme == null) {
    setScheme(Math.min(dots.length - 1, urlParams["sketch"] == "1" ? 5 : 0));
  } else {
